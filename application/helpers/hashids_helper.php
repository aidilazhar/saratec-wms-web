...
require_once FCPATH . './vendor/hashids/hashids/src/Hashids.php';
...
if (!function_exists('hashids_createobject')) {
    function hashids_createobject($salt_ov = NULL, $min_hash_length_ov = NULL, $alphabet_ov = NULL)
    {
        $CI = [
            ...
            'hashids_min_hash_length' => 8,
            'hashids_alphabet' => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
        ];

        $salt               = (!$salt_ov) ? $CI['hashids_salt'] : $salt_ov;
        $min_hash_length    = (!$min_hash_length_ov) ? $CI['hashids_min_hash_length'] : $min_hash_length_ov;
        $alphabet           = (!$alphabet_ov) ? $CI['hashids_alphabet'] : $alphabet_ov;
...
if (!function_exists('hashids_encrypt')) {
    function encode($input, $salt = 'your-salt-string', $min_hash_length = 8, $alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    {
...
if (!function_exists('hashids_decrypt')) {
    function decode($hash, $salt = 'your-salt-string', $min_hash_length = 8, $alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    {
        $hashids    = hashids_createobject($salt, $min_hash_length, $alphabet);
